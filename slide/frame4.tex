...
\begin{frame}{Consistenza generalizzata}
  \begin{definizione}[Consistenza]
    ...
    x_m,\Delta t_m)\to 0$ e  $(x_{n_m},t^{n_m})\to (x,t)$  per
...
Iniziamo la stima per \alert{$v_{j,1}^n+v_{j,2}^n=\sigma(D_j^n)b$} con $b=[1,1]^t$
\[
\begin{aligned}
...
x^{1-\frac{\alpha}{2}}+\Delta x^{\alpha} \\
&\Delta x^{2-\alpha}\to 0\land\Delta x^{1-\frac{\alpha}{2}}\to 0\land
\Delta x^{\alpha}\to 0 \Rightarrow0<\alpha<2
\end{aligned}
\]
