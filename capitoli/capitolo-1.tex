...
Tutti questi problemi hanno portato allo svilupparsi di tecniche \emph{level-set}. Introduciamo alcuni concetti base e riformuliamo la nostra equazione con un \emph{approccio level-set}.%
...
\]
che rappresenta la formulazione level-set dell'equazione \eqref{eq:cp-123}. Inseriamo anche il termine che preserva il volume:
\begin{equation}
  \label{eq:cp-131}
  u_t(x,t)=|Du(x,t)|\left(\Div{\left(\frac{D(x,t)}{|D(x,t)|}\right)}-\frac{\rho\iint\Div{\left(\frac{D(x,t)}{|D(x,t)|}\right)}dx}{3V_0}\right)
\end{equation}
Quindi, ricapitolando, quando una funzione si muove secondo la \eqref{eq:cp-131}, i suoi insiemi di livello si muovono secondo la \eqref{eq:cp-126}. Tale formulazione ci consente di superare i problemi messi in risalto ad inizio paragrafo, infatti:
\begin{itemize}
...
\]
Questo ci dice che il gradiente in $x$ di $u$ è ortogonale al piano tangente a $S$ in $x$. Per la formulazione level-set $u\in C^m$ , $Du\in C^{m-1}$ quindi almeno continua e $Du$ non svanisce intorno a $x_0$; per cio possiamo definire il vetore normale (campo vettoriale unitario di orientazione) come
\[
