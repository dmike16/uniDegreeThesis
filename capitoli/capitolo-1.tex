...
\[
\int_{\Omega}\Div{F(\vec{x})}dV =\int_{\partial\Omega}\left<F(\vec{x}),\vec{n}(\vec{x})\right>d\mu
\]
dove $\vec{n}(\vec{x})$,in tal caso, indica la normale esterna e $dV$,$d\mu$ rispettivamente elemento di volume ed elemento di superficie. Prendiamo come campo vettoriale $F(\vec{x})=\frac{1}{3}\vec{x}$, quindi il volume di $\Omega$ è pari a:
\[
vol(\Omega)=\int_{\Omega}1dV=\int_{\Omega}\Div{(F(\vec{x}))}dV=\frac{1}{3}\int_{\partial\Omega}\left<\vec{x},\vec{n}\right>d\mu,
\]
...
\[
\begin{aligned}
\frac{d}{dt}&\int_{\Omega(t)}F(t,\vec{x})dV(t) = \int_{\Omega(t)}\frac{d}{dt}\left(F(t,\vec{x})dx(t)dy(t)dz(t)\right)=\\
...
\[
\frac{d}{dt}\int_{\Omega(t)}F(t,\vec{x})dV(t)=\int_{\Omega(t)}\frac{\partial F}{\partial t}dV + \int_{\partial \Omega(t)}F\left<\vec{v},\vec{n}\right>d\mu.
\]
Per ottenere il volume nel primo integrale dobbiamo scegliere $F(t,\vec{x})\equiv 1$ e quindi otteniamo la \eqref{eq:cp-124}, eccenzion fatta per il segno in quanto in questo caso $\vec{n}$ rappresenta la normale esterna.
\end{osservazione}
